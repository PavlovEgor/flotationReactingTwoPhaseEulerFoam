...
# gausse = lambda x, A, s: A * np.exp(-x ** 2 / (2 * (s**2)))
# cochy = lambda x, A, s: A * 1 / (s**2 + x**2)
# hype = lambda x, A: A / x
...
#     engine='python'   
# )
# # plt.plot(df["Points:2"][:-2].array, df["U.liquid:1"][:-2].array)
# # plt.plot(df["Points:2"][:-2].array, df["particleMass_inAir"][:-2].array)
# # plt.plot(df["Points:2"][:-2].array, df["U.liquid:1"][:-2].array)
# plt.plot(df["Points:2"][:-2].array, df["alpha.gas"][:-2].array)

# popt, pcov = curve_fit(cochy, df["Points:2"][:-2].array, df["alpha.gas"][:-2].array, p0=[1e-4, 2e-2])
# print(popt)
# grid = np.linspace(0, 0.2, 100)
# plt.plot(grid, cochy(grid, *popt))

# plt.xlim(0, 0.2)
# # plt.ylim(0.4, 1)

# plt.xlabel('Расстояние от центра, м')
# plt.ylabel('Объемная доля воздуха, отн. ед')
# plt.grid(True, which="both", ls="--")  # Сетка для логарифмического масштаба
# plt.show()